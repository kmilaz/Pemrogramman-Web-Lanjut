...
<head>
    ...
    <title>Baby & Kid - POS System</title>
...
<body class="bg-slate-900 text-slate-100 font-sans flex h-screen overflow-hidden">
    ...
    <aside class="w-64 bg-slate-800 border-r border-slate-700 flex flex-col shrink-0">
        ...
        <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
            ...
            <a href="{{ route('category.baby-kid') }}" class="flex items-center gap-3 p-3 bg-blue-600 rounded-xl font-bold shadow-lg shadow-blue-900/20">
    ...
    <main class="flex-1 flex flex-col bg-slate-900 overflow-y-auto">
        <header class="h-16 border-b border-slate-800 bg-slate-900/50 backdrop-blur-md sticky top-0 z-30 flex items-center px-8">
            <h2 class="text-xl font-bold">Baby & Kid</h2>
        </header>

        <div class="p-8">
            <div class="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-6">
                <!-- Static Products -->
                <div class="bg-slate-800 p-4 rounded-3xl border border-slate-700 hover:border-blue-400 shadow-xl group cursor-pointer transition-all">
                    <div class="aspect-square bg-slate-900 rounded-2xl mb-4 flex items-center justify-center overflow-hidden">
                        <i class="fa-solid fa-box-open text-4xl text-blue-300 group-hover:scale-110 transition-transform"></i>
                    </div>
                    <p class="font-bold text-sm">Baby Diapers XL</p>
                    <p class="text-blue-400 font-extrabold mt-1">$15.00</p>
                </div>
                <div class="bg-slate-800 p-4 rounded-3xl border border-slate-700 hover:border-blue-400 shadow-xl group cursor-pointer transition-all">
                    <div class="aspect-square bg-slate-900 rounded-2xl mb-4 flex items-center justify-center overflow-hidden">
                        <i class="fa-solid fa-baby-carriage text-4xl text-pink-300 group-hover:scale-110 transition-transform"></i>
                    </div>
                    <p class="font-bold text-sm">Baby Wet Wipes 80ct</p>
                    <p class="text-blue-400 font-extrabold mt-1">$2.50</p>
                </div>
            </div>
        </div>
    </main>
